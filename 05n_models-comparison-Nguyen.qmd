...
nguy$host <- relevel(nguy$host, "none")
nguy$location <- relevel(nguy$location, "none")
nguy$province <- relevel(nguy$province, "none")
...
#   "symptom ~ inoc_part",
#   "symptom ~ days",
#   "symptom ~ code + province",
#   "symptom ~ code + sample",
#   "symptom ~ code + variety",
#   "symptom ~ variety + province",
#   "symptom ~ variety + sample",
#   "symptom ~ sample + province",
#   "symptom ~ code + province + inoc_part",
#   "symptom ~ code + sample + inoc_part",
#   "symptom ~ code + variety + inoc_part",
#   "symptom ~ code + variety + sample",
#   "symptom ~ variety + province + inoc_part",
#   "symptom ~ variety + sample + inoc_part",
#   "symptom ~ sample + province + inoc_part",
#   "symptom ~ code + province + inoc_part + days",
#   "symptom ~ code + sample + inoc_part + days",
#   "symptom ~ code + sample + inoc_part + province",
#   "symptom ~ code + variety + inoc_part + province",
#   "symptom ~ code + sample + variety + days",
#   "symptom ~ variety + province + inoc_part + days",
#   "symptom ~ variety + sample + inoc_part + days",
#   "symptom ~ sample + province + inoc_part + days",
#   "symptom ~ code + variety + sample + inoc_part + days",
#   "symptom ~ province + variety + sample + inoc_part + days",
#   "symptom ~ code + province + sample + inoc_part + days",
#   "symptom ~ code + province + variety + sample + inoc_part + days")
...
for (i in 1:length(calls)) {
  ...
  models[[n]] <- lm(formula(calls[i]), data = nguy)
}
...
Comparing the different possible models likelihood (AIC and BIC) and R² (see @tbl-lm). There are several models equivalents: 

- `r paste(model_scores$call[lb], collapse = " \n\n - ")`. 

It seems the variables *province*, *sample* and *variety* have simmilar information. 

It is good that *batch* and  *days*  do not improve the model. It shouldn't. 
...
for (i in 1:length(calls)) {
  skip_to_next <- FALSE
  tryCatch(glm.nb(formula(calls[i]), data = nguy), 
    error = function(e) { skip_to_next <<- TRUE})
  if(skip_to_next) { next }   
  n <- paste0("nb", str_pad(i, 2, pad = "0"))
  modelsnb[[n]] <- glm.nb(formula(calls[i]), data = nguy)
}
...
Still need *days*. Only changes *sample* by *province*.

The final residuals are not normally distributed. 
...
for (i in 1:length(calls)) {
  ...
  modelsbi[[n]] <- glm(formula(callsbi[i]), data = nguy, family = binomial)
}
...
It is the same call than for the linear model.  
